(() => {
  ...
  panel.innerHTML = `
    <div class="settings-panel-head providers-panel-head">
      <div>
        <h3>Providers</h3>
        <p>Add OpenAI-compatible, OpenAI Responses, or Anthropic-compatible endpoints to Kilo. Models saved here appear in TL Agent's model selector.</p>
      </div>
    ...
    <form id="providerForm" class="provider-form hidden">
      <div class="provider-form-head">
        <div><strong id="providerFormTitle">Add provider</strong><span>Configuration is saved globally in Kilo and is available across projects.</span></div>
      ...
      <div class="provider-security-note">API keys are stored by Kilo's local auth store. TL Agent does not save provider keys in browser storage or in the provider config file.</div>
      <div class="provider-limit-note">For custom models, set context/output limits when you know them. Kilo disables automatic context compaction when a custom model has no known context limit.</div>
  ...
  const save = async (event) => {
    ...
    try {
      ...
      notice(loaded
        ...
        : `${value.name} was saved, but Kilo did not load ${clean(value.modelID)}. Check the endpoint, protocol, and model ID.`, !loaded);
    } catch (err) {
